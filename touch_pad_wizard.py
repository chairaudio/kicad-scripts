...
class TouchPadWizard(FootprintWizardBase.FootprintWizard):
    ...
    def GetValue(self):
        return "TouchPad-{c}x{r}_{x:g}x{y:g}mm".format(
            r = self.pads['rows'],
            c = self.pads['columns'],
    ...
    def smdTrianglePad(self,module,size,pos,name,up_down=1,left_right=0,flip=1,mask=False):
        ...
        pad.SetPosition(pos)
        pad.SetName(name)
        pad.SetDelta(wxSize(left_right*size[1]*flip,up_down*size[0]*flip))
        return pad
    ...
    def AddStartPadColumn(self,position,touch_width,clearance,name,mask):
        ...
        pad = self.smdTrianglePad(module,size_pad,position,name, 1, 0,-1,mask)
        module.Add(pad)


    # compound a "start pad" shape plus a triangle on the left, pointing to
    # the previous touch-pad
    def AddMiddlePad(self,position,touch_width,clearance,name,mask):
        module = self.module
        size_pad = wxSize(touch_width,touch_width)
        pad = self.smdRectPad(module,size_pad,position,name,mask)
        module.Add(pad)
    ...
    def AddRow(self,pos,row,touch_width,touch_clearance,diogonal_length, name,mask):
        ...
        for n in range(1,row):
            self.AddMiddlePad(pos,touch_width,touch_clearance,name,mask)
